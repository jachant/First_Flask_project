...
def user_create():
    data = request.get_json() # get information
    user_id = len(USERS)
    ...
    if not models.User.is_valid_email(email) or not models.User.is_valid_phone(phone):return Response(status = HTTPStatus.BAD_REQUEST)
...
def generate_expr():
    ...
    operation = data["operation"]# +, -, *, /, ** #todo: random
    if (operation == "random"):
        operation = random.choice(["+", "-", "*", "//", "**"])
    min_num = data["min"]
    max_num = data["max"]

    #todo: check thath count_nouns corresponds to operation
